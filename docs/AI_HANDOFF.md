...
- Production-проверка 25.09.2026: 30-дневная кампания уже запущена, оба origin возвращают `phase: announcement`, `day: 10`, `daysLeft: 20`; `/sw.js` и `/service-worker.js` на обоих доменах отдают `vedelo-custom-sw-v5`. Повторно запускать кампанию или менять её дату не нужно. Device QA установленной PWA остаётся открытым;
...
- реальные E2E интеграций Avito и Telegram Business; работоспособность VK подтверждена владельцем 25.09.2026, отдельная проверка изоляции tenant в этом подтверждении не детализирована;
...
- Для проведённых пополнений баланса и оплат тарифа `Payments.receiptUrl` хранит HTTPS-ссылку
  на чек. Только роль `dev` может сохранить или удалить её через
  ...
  `receipt_missing`; оба используют общий Mongo-фильтр.
...
Историческая локальная приёмка ребрендинга: production build прошёл, 19/19 focused-тестов прошли, Android typecheck прошёл. Актуальный полный ESLint от 25.09.2026 выявил 52 ошибки и 4 предупреждения в существующей рабочей копии, преимущественно правила React hooks; старое указание только одного предупреждения больше не описывает текущее состояние. Точечный ESLint исправлений должен проходить без ошибок.
